...
print("ТЗ: Дан опциональный массив чисел. Если он nil — верни пустой массив. Если есть — верни сумму элементов.")
        ...
        func getThemeWithIf(_ setting: [String: String]?) -> String {
            if let safeTheme = setting?["theme"] {
                return safeTheme
            } else {
                return "light"
            }
        ...
        print(getThemeWithIf(settings)) // dark
        
        // MARK: - 🧩 ЗАДАЧА 1: formatUser (guard let)
        
        print("Напиши функцию, которая принимает опциональные имя и возраст. Если оба есть — верни 'Имя: <имя>, Возраст: <возраст>'. Если нет — 'Данные неполные'.")
        
        // ✅ Вариант 1: guard let + !isEmpty -> проверяет: nil и пустую строку.

        func formatUserVar1(name: String?, age: Int?) -> String {
            guard let safeName = name,!safeName.isEmpty,
                  let safeAge = age else { return "Data is incomlete"
            }
            return "Name: \(safeName), age: \(safeAge)"
        }
        print(formatUserVar1(name: "Vale.ponick", age: 59)) // Name: Vale.ponick, age: 59
        print(formatUserVar1(name: "", age: 0)) // Data is incomlete
        print(formatUserVar1(name: "Vale.ponick?", age: nil)) // Data is incomlete
        
        // ✅ Вариант 2: ?? + isEmpty
        // Проверяет: пустую строку, но nil уже заменён на "" и 0
        
        func formatUserVar2(name: String?, age: String?) -> String {
            let safeName = name ?? ""
            
            guard !safeName.isEmpty,
                  let ageString = age, !ageString.isEmpty,
                  let ageNum = Int(ageString) else {
                return "Data is incomlete"
            }
            return "Name: \(safeName), age: \(ageNum)"
        }
