...
struct StructAndClassPractics {
    static func run() {
        ...
        print("4️⃣: 'Исследовательский Зонд'. Создай класс SpaceProbe, 2 свойства (var): serialNumber + destination (цель полета). Напиши func runTask4(), внутри создай первый зонд через константу let с № 2112 и целью 'Mars'.Создай второй зонд через константу let, просто приравняв его к первому.Измени у второго зонда цель полета (destination) на 'Jupiter'.Измени у первого зонда серийный № (serialNumber) на 8888. print в консоль № и цель полета второго зонда.")
     ...
     struct EnemyShip {
       ...
       let dangerousSectors: [String] = ["Sector 001", "Neutral Zone", "Klingon Space"] // Черный список опасных секторов
        ...
        /**
         📊 ИНЖЕНЕРНЫЙ ОТЧЕТ: МОДУЛИ 7, 8, 9 (ИНДЕКСЫ И МУТАЦИИ)✅ ПЛЮСЫ (Что сделано круто)❌ ТОЧКИ РОСТА / ОШИБКИ (На что ругался тимлид)Железный автоматизм в firstIndex: Четко разделены of: для простых массивов строк/чисел и where: для сложных структур.Бизнес-логика в логах: Текст строки sent to spase Station противоречит смыслу метода deployComponent (компонент улетает с нее, а не на нее).Исправление Swift 6 дефектов: Код полностью адаптирован под стабильный Swift 5 без использования несовместимых методов.Копипаст в тест-блоке: Повторение одинаковых конструкций if let два раза подряд вместо написания одной переиспользуемой логики.Гибкость аргументов: Устранена жесткая привязка к цифрам (хардкод 20), функция aimAtTargets теперь принимает динамический параметр.Именование (Shadowing): Параметр функции shieldPower назван так же, как свойство структуры, что может запутать при чтении кода.Чистота консоли: Все опционалы извлечены безопасно (guard let, ?? 0), компилятор выдает 0 ворнингов.Опечатки в строках: Мелкие баги в английском тексте (spase вместо space).🔧 Как сделать этот код идеальным (Middle-стиль):Если захочешь обновить функции, вот как выглядит технически безупречный вариант без этих замечаний:swift// Изменен аргумент на maxShield, исправлен текст и опечатка
         func aimAtTargets(maxShield limit: Int) -> Int? {
