...
struct StructAndClassPractics {
    static func run() {
        ...
        print("2️⃣: «Смена капитана» Внутри метода run(), ниже первой задачи, создай класс Commander (Командир), одно изменяемое свойство: name.Напоминание: Классу обязательно нужен инициализатор init(name: String) { self.name = name }, иначе компилятор выдаст ошибку.Напиши функцию runTask2(), внутри которой:Создай первого командира с именем 'James Kirk'. Подумай, что тут лучше использовать — let или var?Создай второго командира через var, просто приравняв его к первому.Измени у второго командира имя на 'Spock'.Напечатай в консоль имя первого командира.")
